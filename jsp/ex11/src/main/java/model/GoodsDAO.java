...
public class GoodsDAO {
	...
	public ArrayList<GoodsVO> list(QueryVO query,String uid){
		...
		try {
			...
			System.out.println("좋아요 목록 : "+ e.toString() );
			}
		return array;
		}
	
	public ArrayList<GoodsVO> slist(QueryVO query,String uid,String gid){
		ArrayList<GoodsVO> array = new ArrayList<GoodsVO>();
		try {
			String sql="select *,";
			sql +="(select count(*) from favorite f where uid=? and f.gid=g.gid) ucnt,";
			sql +="(select count(*) from favorite f where f.gid=g.gid) fcnt";
			sql +=" from goods g";
			sql +=" where title like ? and not gid=?";
			sql +=" order by regdate desc";
			sql +=" limit ?, ?";
			PreparedStatement ps = con.prepareStatement(sql);
			ps.setString(1, uid);
			ps.setString(3, gid);
			ps.setString(2, "%"+query.getWord()+"%");
			ps.setInt(4, (query.getPage()-1)*query.getSize());
			ps.setInt(5, query.getSize());
			ResultSet rs= ps.executeQuery();
			while(rs.next()) {
				GoodsVO vo = new GoodsVO();
				vo.setGid(rs.getString("gid"));
			...
			System.out.println("비슷한 상품추천 : "+ e.toString() );
			}
		return array;
		}
}