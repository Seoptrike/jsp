...
public class OrderDAO {
	...
	public ArrayList<PurchaseVO> list(QueryVO query){
		...
		try {
			...
			ps.execute();
			 ...
			 System.out.println("모든 주문 목록:"+e.toString());
			}
		return array;
	}
	//특정 주문의 상품목록
	public ArrayList<OrderVO> olist(String pid){
		...
		try {
			...
			ps.execute();
			 ...
			 System.out.println("특정유저의 주문상품목록"+e.toString());
			}
		return array;
	}
	...
	public void insert(PurchaseVO vo) {
		...
		ps.setString(5, vo.getRaddress1());
		ps.setString(6, vo.getRaddress2());
		ps.setInt(7, vo.getSum());
		ps.execute();

		}catch(Exception e) {
		 System.out.println("주문자정보등록"+e.toString());
		}
	}
}
