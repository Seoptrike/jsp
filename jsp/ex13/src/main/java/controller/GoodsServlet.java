...
@WebServlet(value= {"/goods/search","/goods/search.json","/goods/insert","/goods/list","/goods/list.json","/goods/delete","/goods/total","/goods/read","/goods/slist.json"})
public class GoodsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "/goods/list":
			...
			dis.forward(request, response);
			break;
		case "/goods/list.json": //테스트 /goods/list.json?w
			Gson gson = new Gson();
		...
		case "/goods/slist.json": //테스트 /goods/list.json?
			gson = new Gson();
			query=new QueryVO();
			...
			query.setPage(Integer.parseInt(request.getParameter("page")));
			query.setSize(Integer.parseInt(request.getParameter("size")));
			out.print(gson.toJson(dao.slist(query,uid,gid)));
			break;	
